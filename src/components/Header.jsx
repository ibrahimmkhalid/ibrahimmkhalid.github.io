...
export default function Header() {
  return (
    <div className="sticky top-0 z-10 w-full overflow-x-hidden border-b border-gray-200 bg-white/95 backdrop-blur">
      <header
        className="mx-auto flex min-h-14 max-w-6xl flex-wrap items-center gap-x-6 gap-y-2 px-4 py-3 sm:px-6"
        id="#"
      >
        <span
        ...
        <>
          <nav className="flex w-full min-w-0 gap-4 overflow-x-auto text-sm whitespace-nowrap text-gray-700 sm:gap-6 md:ml-auto md:w-auto md:text-base">
            {tabs.map((tab, idx) => (
              <span
                key={idx}
